...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/enhance-prompt", requireAuth, async (req, res) => {
    try {
      ...
      const result = await enhancePromptWithAI(prompt);
      
      // Update user's enhancements usage count  
      await storage.updateUserEnhancementUsage(req.user!.id, (req.user!.enhancementsUsed || 0) + 1);
  ...
  app.post("/api/enhance-prompt", async (req, res) => {
    ...
    const userLimit = enhancementLimits[user.plan];
    if (user.enhancementsUsed >= userLimit) {
      return res.status(429).json({ 
        error: "Enhancement limit reached", 
        message: `You've reached your ${user.plan} plan limit of ${userLimit} enhancements. Upgrade to continue.`
      });
    }
...
Respond in this exact JSON format:
{
  ...
  "improvements": ["improvement 1", "improvement 2", "improvement 3"]
      ...
      try {
        ...
        await storage.updateUserEnhancementUsage(user.id, user.enhancementsUsed + 1);
